        ...
        foreach($allowed as $status_val) {
            ...
            foreach($payment_methods as $payment){
                if(file_exists('../modules/'.$payment['pay_name'].'/logo.gif')) {
                    $output .= '<img src="'.__PS_BASE_URI__.'modules/'.$payment['pay_name'].'/logo.gif" width="16" title="'.$payment['name'].'" alt="'.$payment['pay_name'].'" style="vertical-align:middle" />';
                }else {
                    $output .= ''.$payment['pay_name'].'';
                }
                $checked = false;
                ...
                $output = $output.' <input type="checkbox" name="data['.$status_val.'][0]['.$payment['id_module'].']" value="1" '.($checked ? 'checked="checked"' : '').' /> ('.$payment['pay_name'].')<br />';
            }
